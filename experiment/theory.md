...
During the forward pass, the hidden states $H_1, H_2, \ldots, H_\tau$ at each time step must be stored so that they can be reused during the backward pass. As a result, the memory requirement is also $\mathcal{O}(\tau)$. The application of the backpropagation algorithm to the unrolled RNN computation graph is known as **Backpropagation Through Time (BPTT)**.
...
- **Parameter Sharing:**
  RNNs share the same set of parameters $\{W_{xh}, W_{hh}, W_{ho}, b_h, b_o\}$ across all time steps, which reduces the number of parameters that need to be learnt and can lead to better generalization.
...
- **Vanishing Gradient:**
  During backpropagation, gradients diminish as they pass through each time step, leading to minimal weight updates. Formally, the gradient $\partial L / \partial H_t$ involves products of Jacobians that shrink exponentially with $\tau$, limiting the RNN's ability to learn long-term dependencies crucial for tasks like language translation.

- **Exploding Gradient:**
  Sometimes gradients grow uncontrollably, causing excessively large weight updates that destabilize training. This is the problem of exploding gradients, often mitigated by gradient clipping: $\hat{g} \leftarrow g \cdot \min\!\left(1,\, \theta / \|g\|\right)$.

- **Lack of Parallelism:**
  RNNs are inherently sequential — computing $H_t$ requires $H_{t-1}$ — making it difficult to parallelize the computation. This limits the speed and scalability of the network compared to architectures like Transformers.