...
const RNNAnimation = {
    ...
    renderArrow: function(index) {
        return `
            <div class="rnn-arrow-container" data-index="${index}">
                <svg viewBox="0 0 80 32" class="rnn-arrow-svg">
                    <defs>
    ...
    scrollToCurrentStep: function() {
        // Get the scrollable container (rnn-sequence-wrapper)
        const scrollContainer = document.querySelector('.rnn-sequence-wrapper');
        ...
        const currentCell = cells[this.currentStep];
        if (!currentCell) return;
        
        // Calculate the scroll position to center the current cell
